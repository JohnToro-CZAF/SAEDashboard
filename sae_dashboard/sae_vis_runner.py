...
class SaeVisRunner:
    ...
    def run(
        ...
        features_list = self.handle_features(self.cfg.features, encoder) #* default all features if none provided
        feature_batches = self.get_feature_batches(features_list) # * split features into batches
        progress = self.get_progress_bar(tokens, feature_batches, features_list) # * create progress bars

        feature_data_generator = FeatureDataGeneratorFactory.create(
            self.cfg, model, encoder, tokens
        ) # * create the feature data generator [takes in model, SAE, tokens[nrwos, context_length]]
        # * get_feature_data methods are called here
        # * passing a batch features that we want to collect information at here
        # * returns, feature activation, residuals and correlation coefficients of other neurons or other features
        ...
        for features in feature_batches:
            ...
            # * all_feat_acts: [batch, seq, features] => activations of all features
            # * feature_resid_dir: [features, d_model] => direction of feature on the residual stream
            # * feature_out_dir: [features, d_model] => direction of feature in the output
            # * corrcoef_neurons: [features, d_in] => correlation between features and din neurons of model
            # * corrcoef_encoder: [features, features] => correlation between features and features
            ...
            ) # * get statistics of the features, putting in range 
            ...
            # print("time for FeatureStatistics.create", time.time() - t0)

            # ! Data setup code (defining the main objects we'll eventually return)
            feature_data_dict: dict[int, FeatureData] = {
                feat: FeatureData() for feat in features #* each feature we will assign FeatureData object which contains all 
                # information about the feature for visualization on html
            }
            t0 = time.time()
            ...
            ) # * get feature table data, which contains the correlation between features and neurons, features and features
            # * this will be used in the visualization, par of feature_data_dict
            ...
            t0 = time.time()
            for i, (feat, logit_vector) in enumerate(zip(features, logits)): # logits [features, d_vocab]
                ...
                # print("masked_feat_acts.shape", masked_feat_acts.shape)
                # print("logits[i].shape", logits[i].shape)
                # print("feature_resid_dir[i].shape", feature_resid_dir[i].shape)
                ...
                # print("sequence_data", feature_data_dict[feat].sequence_data)
                if self.cfg.use_dfa:
                ...
                if progress is not None:
                    progress[1].update(1)
            # print("time for all features statistics", time.time() - t0)
    ...
    def get_progress_bar(
        ...
        # print(f"n_token_batches: {n_token_batches}")
        # print(f"len(feature_batches): {len(feature_batches)}")
        # print(f"len(tokens): {len(tokens)}")
        # print(f"cfg.minibatch_size_tokens: {self.cfg.minibatch_size_tokens}")
