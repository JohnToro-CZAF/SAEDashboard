...
class FeatureStatistics:
    ...
    def create(
        ...
        for r, p in ranges_and_precisions:
            start, end = r
            step = 10**-p # 10^-5 = 0.00001, 0.99 -> 1.0  there are 10000 bins quantiles
        ...
        for i in range(0, n_features, batch_size):
            ...
            _max.extend(batch.max(dim=-1).values.tolist()) # choose the max value of token for each feature in the data [n_rows * 1024]
            ...
            # print("quantiles_tensor", quantiles_tensor.size())
            # chunking down the input to avoid OOM errors
            batch_quantile_data = torch.quantile(
                batch.to(torch.float32),
                quantiles_tensor.to(torch.float32),
                dim=-1,
            ) #* putting the tokenn activation for each feature into corresponding quantile bin
            # print("batch_quantile_data", batch_quantile_data.size())
