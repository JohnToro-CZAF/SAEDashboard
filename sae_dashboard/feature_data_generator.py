...
class FeatureDataGenerator:
    ...
    def get_feature_data(  # type: ignore
        ...
        feature_out_dir = self.encoder.W_dec[feature_indices]  # [feats d_autoencoder], vector direction for each feature
        feature_resid_dir = to_resid_direction(
            feature_out_dir, self.model
        )  # [feats d_model] vector direction for each feature to the residual stream, itself in feature_out_dir space if
        # encoder is trained after residual stream

        # ! Compute & concatenate together all feature activations & post-activation function values
        outter_loop_time = time.time()
        for i, minibatch in enumerate(self.token_minibatches):
            t0 = time.time()
            minibatch.to(self.cfg.device)
            model_activation_dict = self.get_model_acts(i, minibatch) # * model's activation after the hooked, should be the place
            # * when SAE is trained on
            ...
            with FeatureMaskingContext(self.encoder, feature_indices):
                ...
                # print("feature_acts", feature_acts.shape)
            # print("encode_time", time.time() - t0)
            t0 = time.time()
            # * primary_acts: model's hidden state per token
            # * feature_acts: SAE's activations on each feature per token
            self.update_rolling_coefficients(
                model_acts=primary_acts, # [batch seq d_in]
                feature_acts=feature_acts, # [batch seq feats]
                corrcoef_neurons=corrcoef_neurons,
                corrcoef_encoder=corrcoef_encoder,
            )

            # Add these to the lists (we'll eventually concat)
            all_feat_acts.append(feature_acts) # * [batch seq feats]
...
class FeatureMaskingContext:
    ...
    def __enter__(self):
        ...
        elif self.sae.cfg.architecture == "topk":
