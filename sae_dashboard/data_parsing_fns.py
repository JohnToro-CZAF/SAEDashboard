...
def parse_prompt_data(
    ...
    print(".....Processing prompt data....")
    for i, feat in tqdm.tqdm(enumerate(feature_idx)):
        ...
        contribution_to_logprobs = contribution_to_logprobs.to(torch.float32)
        top_contribution_to_logits = TopK(contribution_to_logprobs[:-1].to(torch.float32), k=5)
    ...
    scores_dict: dict[str, tuple[list[int], list[str]]] = {}
    print(".....Processing prompt data per token....")
    for seq_pos, seq_key in tqdm.tqdm(enumerate([f"{t!r} ({i})" for i, t in enumerate(str_toks)])):
...
def get_prompt_data(
    ...
    last_layer = 0
    for k, v in model.hook_dict.items():
        if 'blocks' in k:
            layer_id = int(k.split('.')[1])
            last_layer = max(last_layer, layer_id)
    last_layer_resid_post_hook_point = f"blocks.{last_layer}.hook_resid_post"
    model_wrapped = TransformerLensWrapper(model, ActivationConfig(cfg.hook_point, [last_layer_resid_post_hook_point]))
    ...
    model_activation_dict = model_wrapped(tokens, return_logits=False)
    # print(model_activation_dict.keys())
    hook_acts = model_activation_dict[model_wrapped.activation_config.primary_hook_point]
    resid_post = model_activation_dict[last_layer_resid_post_hook_point]
    # act_post = encoder.encode(hook_acts)  # [seq d_model]
    # print("hook_acts shape: ", hook_acts.shape)
    
    with FeatureMaskingContext(encoder, feature_idx):
        feat_acts = encoder.encode(hook_acts).to(
            DTYPES[sae_vis_data.cfg.dtype]
        )
    # print("feat_acts shape: ", feat_acts.shape)
    # rotate [B, 52, 3] -> [B, 3, 52]
    # feat_acts = feat_acts.permute(0, 2, 1)
    feat_acts = feat_acts.squeeze()
    # print("feat_acts shape: ", feat_acts.shape)
    # resid_post, act_post = model_wrapped(tokens, return_logits=False)
    resid_post: Tensor = resid_post.squeeze(0)
    # print("resid_post shape: ", resid_post.shape)
    ...
    scores_dict = parse_prompt_data(
        ...
        feat_acts=feat_acts.to(DTYPES[sae_vis_data.cfg.dtype]).to(torch.float32),
        feature_resid_dir=feature_resid_dir.to(DTYPES[sae_vis_data.cfg.dtype]).to(torch.float32),
        resid_post=resid_post.to(DTYPES[sae_vis_data.cfg.dtype]).to(torch.float32),
        W_U=model.W_U.to(DTYPES[sae_vis_data.cfg.dtype]).to(torch.float32),
